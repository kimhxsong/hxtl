...
class __tree_iterator {
 ...
 private:
  ...
  friend class __tree;  // explicit TreeIterator(node_pointer);
                        // template <class, class> friend class __tree_iterator;
...
class __tree_const_iterator {
 ...
 private:
  ...
  explicit __tree_const_iterator(const_node_pointer np) : np(np) {}
...
class __tree {
 public:
  ...
  __tree(InputIterator first, InputIterator last, const value_compare& comp = value_compare(),
         const allocator_type& alloc = allocator_type())
      : alloc(alloc), root(0), begin_node(value_type()), end_node(value_type()), sz(0), comp(comp) {
    while (first != last) {
      this->insert(*first++);
