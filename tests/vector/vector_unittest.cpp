...
TEST(vector_modifiers, insert) {
  ...
  __ft__vec2.insert(__ft__vec2.begin(), __ft__vec.begin(),
                    __ft__vec.end());  // range(3)
...
TEST(vector_modifiers, erase) {
  ...
  ft::vector<int>::iterator __ft__it =
      __ft__vec.erase(__ft__vec.begin() + 1, __ft__vec.begin() + 3);
  std::vector<int>::iterator __std__it =
...
void	checkErase(TESTED_NAMESPACE::vector<TESTED_TYPE> const &vct,
                                        TESTED_NAMESPACE::vector<TESTED_TYPE>::const_iterator const
&it)
{
        static int i = 0;
  ...
  for (unsigned long int i = 0; i < vct.size(); ++i) vct[i] = (vct.size() - i) * 3;
  printSize(vct);
